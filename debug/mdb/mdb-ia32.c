...
unsigned long MajorVersion = 2622;
unsigned long MinorVersion = 9042008;
unsigned long BuildVersion = 2;
...
void ReadStackFrame(void *frame, StackFrame *sf, unsigned long processor)
{
   ...
   sf->tESI = regs->esi;
   sf->tEDI = regs->edi;
   sf->tES = (unsigned short)regs->xes;
   sf->tCS = (unsigned short)regs->xcs;
   sf->tDS = (unsigned short)regs->xds;
   ...
   {
      sf->tESP = regs->esp;
      sf->tSS = (unsigned short)regs->xss;
   }
   /* save state. */
